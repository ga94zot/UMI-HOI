...
# Attribution: derived from PViC (https://github.com/fredzzhang/pvic, Frederic Zhang,
# BSD-3-Clause, see LICENSE); names are kept as in PViC. UMI-HOI changes: DataFactory /
# HICODet loading of VLM verb answers and vision tokens, custom_collate, W&B logging in
# CustomisedDLE (_on_start, _print_statistics, _on_end_epoch), zero-shot seen/unseen
# evaluation in _on_end_epoch, VLM inputs threaded through test_hico / test_vcoco / cache_*.
    ...
    def _on_start(self):
        if self._train_loader.dataset.name == "hicodet":
            ...
            if self._rank == 0:
                ...
                # Zero-shot train types (RF_UC/NF_UC/UO/UV) report seen/unseen instead of
                # rare/non-rare; declare those metrics too so W&B tracks their max as well.
                wandb.define_metric("mAP seen", step_metric="epochs", summary="max")
                wandb.define_metric("mAP unseen", step_metric="epochs", summary="max")
        ...
        if self._train_loader.dataset.name == "hicodet":
            ...
            if self._rank == 0:
                ...
                if self.test_dataloader.dataset.dataset.train_type == 'default':
                    ...
                    #     "mAP rare": perf[1], "mAP non_rare": perf[2]
                    # })
                    # perf[1]/perf[2] are seen/unseen mAP here (see the print above), but were
                    # logged under the rare/non_rare keys, so zero-shot W&B charts were mislabeled.
                    wandb.log({
                        "epochs": self._state.epoch, "mAP full": perf[0],
                        "mAP seen": perf[1], "mAP unseen": perf[2]
                    })
        else:
            ap = self.test_vcoco()
            if self._rank == 0:
