...
# Attribution: derived from PViC `transformers.py` (https://github.com/fredzzhang/pvic,
# Frederic Zhang, BSD-3-Clause, see LICENSE), which in turn adapts DETR and Swin code as
# noted above. In UMI-HOI this file was `transformers_.py` (underscore to avoid clashing
# with the HuggingFace package) until 2026-09-09 and is now `umi_transformers.py`; class
# names are kept as in PViC.
# UMI-HOI changes in this file: TransformerDecoderLayer / TransformerDecoder implement the
# unified multimodal tokens (VLM verb answers via llava_cali_embedding, SigLIP 2 vision tokens
# via clip_token_proj_*, register embeddings) and the Semantic Multi-Head Attention head
# split (sub_headnum / obj_headnum). SelfAttnBlock, CrossAttnBlock, Expert, TopkMoE,
# TransformerDecoderLayerNoTaskSpecified and TransformerDecoderNoTaskSpecified are
# UMI-HOI additions (experimental; not used by umi.build_detector).
import copy
import math
import os
import torch
...
from umi_attention import MultiheadAttention, MultiheadAttentionSigmoid
from typing import List, Optional, Callable

# 2026-09-01: input dim of the VLM visual-token projections, previously
# hardcoded as 1536 (SigLIP 2 giant's hidden size) in four nn.Linear layers
...
# like the earlier CLIP->SigLIP 2 swap but parameterised instead of re-edited.
...
# UMI-HOI addition (not in upstream PViC).
...
class CrossAttnBlock(nn.Module):
    ...
    def forward(self, cls_tokens, input_tokens, input_masks):
        ...
        x = self.self_attn_ln2((x + self.self_attn_dp2(self.self_attn_ffn(x))).permute(0,2,1)).permute(0,2,1)
        return x
# UMI-HOI addition (not in upstream PViC); experimental MoE, currently unused.
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, q_dim, kv_dim, num_heads, ffn_interm_dim, dropout=0.1, layer_id=0, sub_headnum=2, obj_headnum=6):
        ...
        # Rationale: the ablation in the paper attributes SigLIP 2 > CLIP to
...
# UMI-HOI addition (not in upstream PViC); experimental variant, not used by build_detector.
