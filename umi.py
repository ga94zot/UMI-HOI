...
# Attribution: derived from PViC `pvic.py` (https://github.com/fredzzhang/pvic, Frederic
# Zhang, BSD-3-Clause, see LICENSE). This file was `pvic.py` in UMI-HOI until 2026-09-09;
# it is renamed `umi.py` and the top-level model class `PViC` is renamed `UMI` because the
# model here is the UMI-HOI detector. All other class/function names are kept as in PViC.
# UMI-HOI changes in this file: UMI.forward / advanced_forward /
# compute_classification_loss take the VLM verb answers and vision tokens
# (llava_answer, llava_feature); build_detector and the module constructors pass the
# S-MHA head split (sub_headnum / obj_headnum); FeatureHeadNoTaskSpecified and
# HumanObjectMatcherNoTaskSpecified are UMI-HOI additions.
...
# -- SigLIP 2 tokens concatenated with a token-aligned DINOv3 dump (both 24x24
...
# FeatureHead's backbone input; the decoder's VLM tokens stay pure SigLIP 2.
...
# a cosine classifier against frozen SigLIP 2 text embeddings of the 117 verb
...
class UMI(nn.Module):
    """UMI-HOI detector (PViC two-stage HOI detector + unified multimodal tokens + S-MHA)"""
    ...
    def forward(self,
        ...
        with torch.no_grad():
            if self.detector_type == "advanced":
                ...
                # FeatureHead; the decoder keeps the pure SigLIP 2 slice.
