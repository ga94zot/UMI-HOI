...
```bash
# Training
DETR=base python main.py --pretrained checkpoints/detr-r50-hicodet.pth --output-dir outputs/umi-detr-r50-hicodet --llava-answer-path text_folder --llava-token-path token_folder --repr-dim 512 --sub-headnum 5 --obj-headnum 3 --train-type default --world-size 1 --batch-size 1 --epochs 20 --lr-drop 7
# Testing
...
```bash
# Training
DETR=advanced python main.py --pretrained checkpoints/defm-detr-r50-dp0-mqs-lft-iter-2stg-hicodet.pth --output-dir outputs/umi-defm-detr-r50-hicodet --llava-answer-path text_folder --llava-token-path token_folder --repr-dim 512 --sub-headnum 5 --obj-headnum 3 --train-type default --world-size 1 --batch-size 16 --epochs 20 --lr-drop 7
...
### UMI-H-Defm-DETR-SwinL
```bash
# Training
DETR=advanced python main.py --backbone swin_large --use-checkpoint --drop-path-rate 0.5 --num-queries-one2one 900 --num-queries-one2many 1500 --pretrained checkpoints/h-defm-detr-swinL-dp0-mqs-lft-iter-2stg-hicodet.pth --output-dir outputs/umi-h-defm-detr-swinL-hicodet --llava-answer-path text_folder --llava-token-path token_folder --repr-dim 512 --sub-headnum 5 --obj-headnum 3 --train-type default --world-size 1 --batch-size 1 --epochs 20 --lr-drop 7
...
```bash
# Training
DETR=base python main.py --dataset vcoco --data-root vcoco/ --partitions trainval test --pretrained checkpoints/detr-r50-vcoco.pth --output-dir outputs/umi-detr-r50-vcoco --llava-answer-path text_folder --llava-token-path token_folder --repr-dim 512 --sub-headnum 5 --obj-headnum 3 --train-type default --world-size 1 --batch-size 16 --epochs 20 --lr-drop 7
...
```bash
# Training
DETR=advanced python main.py --dataset vcoco --data-root vcoco/ --partitions trainval test --backbone swin_large --use-checkpoint --drop-path-rate 0.5 --num-queries-one2one 900 --num-queries-one2many 1500  --pretrained checkpoints/h-defm-detr-swinL-dp0-mqs-lft-iter-2stg-vcoco.pth  --output-dir outputs/umi-h-defm-detr-swinL-vcoco --llava-answer-path text_folder --llava-token-path token_folder --repr-dim 512 --sub-headnum 5 --obj-headnum 3 --train-type default --world-size 1 --batch-size 16 --epochs 20 --lr-drop 7
# Caching detections
