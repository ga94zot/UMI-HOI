...
# BSD-3-Clause, see LICENSE). UMI-HOI changes: --llava-answer-path / --llava-token-path,
# --train-type zero-shot splits, --sub-headnum / --obj-headnum (S-MHA), --use-wandb.
...
from umi import build_detector
...
def main(rank, args):
    ...
    if os.path.exists(args.resume):
        print(f"=> Rank {rank}: UMI loaded from saved checkpoint {args.resume}.")
        checkpoint = torch.load(args.resume, map_location='cpu')
        model.load_state_dict(checkpoint['model_state_dict'])
    else:
        print(f"=> Rank {rank}: UMI randomly initialised.")
